...
## 2026-09-15 · chem_lab7.2/7.3 游戏包升级（合成大元素 v3 + 轨道之门 v2）

- 同名覆盖两个游戏页；交付件已自带误触保护 + 右下角返回首页（游戏不留返回门户，符合既定规则），**零改动直接上线**
- **约定变化**：交付件内已预嵌备案角落小字（chat 侧学会了 B+ 格式）——7.2/7.3 在仓库里就带备案行，pages.dev 也会显示（无害且合规）；B+ 注入脚本已改为**幂等**（已有 beian 则跳过）
- 7.2 v3：元素球表情、合并反馈、≥16 号震屏、放射性衰变（43/61/83+）、WebAudio 音效、Z 求和速查表、轨道之门联动加成
- 7.3 v2：音效六件套、路径前瞻条、连击系统、得分/纪录持久化、死亡教学卡、混元4 能量柱贴图、熔炉联动跳级（localStorage zaoFusionMaxZ）
- 旧版已归档「过渡文件已舍弃」；pages.dev curl 验收：v3/v2 标记与文件大小全对
- **⚠️ zao 部署挂起**：2026-09-15 15:00 起宝塔面板全路径 404（TCP 通、HTTP 404，安全入口疑似变更/面板异常），v11 包已就绪于工作区 zao-chem-dist-v11.zip，面板恢复后按既有 API 流程一条命令部署


---

## 2026-09-15（下午）· chem_lab5.2 换代为离子塔防 3D v2（commit 8805a5e）

- 元素防线 → **离子塔防 3D**：程序化玻璃器皿塔（锥形瓶/滴定管/圆底烧瓶）、fresnel 发光离子球敌人、抛物线弹丸+尾迹、3 级升级/出售、5 张主题地图（实验室/矿井/电解/熔炉/湿地）
- **首次引入素材目录**：`public/teaching/assets/td/`（50 文件 ~8MB，混元 AI 生成，必须与 html 同级），scripts/README 已注明
- 首页卡片改名「离子塔防」+ 描述更新；旧版与 build_td.py 归档「过渡文件已舍弃」
- 交付件未带误触保护，已补注入；返回首页自带（游戏无返回门户，合规）
...
## 2026-09-15（深夜）· 复习板块新增「造·化学」+ 三模块改名

- **chem_lab2.4 造·化学虚拟实验室**：10 类原创实验任务（配制/中和滴定/中和热/粗盐提纯/金属速率/溶解度曲线/密度鉴别/气体计量/沉淀），器皿多规格自由摆放、三种移液精度、气泡/沉淀/指示剂现象层、星级评分+积分（localStorage zclab_*）。适配：three.js 本地 vendor 优先（cdnjs 回退）、补「← 门户 + ⌂ 首页」双键（复习板块惯例）、误触保护、预嵌备案行
- **复习板块三模块改名（主题/内容不变）**：知识球·知识地图→**知识星图**、化学闪卡复习→**记忆回廊**、知识挑战树→**挑战之树**；门户卡片、页内 title/h1、交叉链接（2.1→2.2、2.3→2.1）全部同步
- zao v14（113 文件 10191KB）验收：root hash=index-obfFAWz8.js 与包一致；lab2.4 全要素在；lab2.1 新名生效
- push 仍挂起（github.com SNI 阻断未恢复），本地 commit 待推


---

## 2026-09-17 · VSEPR 实验室换代 v2.0（commit 4ac58b8）

- chem_lab1.1 整页替换：33 分子/离子（sp–sp³d³ 全类型，含苯环/S₈）、成键四幕动画（滑杆控制、核云同步、终态无残影）、双视角、封盘推演答题（连击+XP 写入 zao_passport_* 统一前缀）、键角量角器、三主题、three.js r160 **内联**（零外部依赖，断网可用）
- 适配：顶栏演示死链 → 真实路由（logo/首页→/、数据库→/database、生活馆→/life、教学实验→/teaching），误触保护、预嵌备案行；旧版归档「过渡文件已舍弃/chem_lab1.1-VSEPR-v1」
- 门户卡片描述同步：184 种 → 33 种 + 新特性
- zao v15（113 文件 10193KB）验收：hash=index-C-3he-fd.js 一致；浏览器运行时探针：THREE 在位、33 分子下拉、切 H2O + 滑杆无报错、首页链接正确、备案行在
- push 仍挂起（github.com SNI 阻断第 3 天），本地 5 个 commit 待推


---

## 2026-09-18 · 异构迷城换代 + 板块调整 + 论坛举报入口（commit 2068719）

- **chem_lab3.3 异构迷城**替换同分异构体闯关：150 题库（每轮随机 5-8 题）、错题本、每日挑战（种子同日同题）、称号墙（4 档）、连对奖励/全对 +50、成绩分享卡 PNG、76+ 题 2D 结构式（价态校验过）、12 分子星座星空背景（three r128 内联零依赖）。适配：返回门户改相对路径 + 补返回首页、预嵌备案行；旧版与 build_isomer_lab.py 归档「过渡文件已舍弃/chem_lab3.3-同分异构体闯关」
- **板块调整**：Ⅲ 化学反应原理整体隐藏（4 个模块文件保留未删，仅下架门户卡片），改名「Ⅲ 其他」（OTHERS · 实验 · 创造），造·化学从 Ⅳ 挪入 Ⅲ；Ⅳ 复习板块恢复 3 卡片
- **论坛举报入口**：帖子列表下方固定「发现违规内容请举报至：1298052658@qq.com」（mailto）
- zao v16（113 文件 10360KB）验收：hash=index-BSUrHoFY.js 一致；浏览器探针：THREE r128、导航四键齐、章节渲染、品牌异构迷城、备案在；论坛 mailto 链接在位、帖子正常加载
- push 成功（GitHub 已恢复），pages.dev 自动部署


---

## 2026-09-18（补）· 门户残留改名 + 单卡宽度修正

- 有机板块 03 卡片：同分异构体闯关挑战 → **异构迷城**（desc 同步为 150 题库新特性）
- `.tk-cards.c2` 网格 `auto-fit` → `auto-fill`：单卡片板块（Ⅲ 其他）不再拉伸占满整行，与其他模块同宽、右侧留空；多卡板块布局不变
- zao v17 部署验收：hash=index-BX12FxET.js；浏览器探针：旧名清零、四板块（结构专题/有机化学/其他/复习板块）、网格 276px 固定列宽


---

## 2026-09-19 · 记忆回廊 3D 版上线（chem_lab2.4）+ 挑战之树下线

- **chem_lab2.4 记忆回廊**（5.4MB 单文件，three r128 内联）：可行走三维回廊、十座记忆门（跟随目光转向）、页内翻卡复习 + 三阶闯关练习、尽头记忆之光；读取 chem_lab2.2 作数据源（失败回退内嵌卡数据）；存档键 chem-fc-progress-v1 / chem-kt-progress-v1 **保持不变**，老用户进度无缝。适配：crumb 补首页链接 + 同步新命名（知识星图/闯关练习）、移动抽屉 mQuest 死链 2.3→#（JS 拦截走页内）、误触保护、预嵌备案行
- **挑战之树下线**：chem_lab2.3.html + build_challengetree.py 归档「过渡文件已舍弃」，服务器旧文件已删
- **造·化学挪位**：chem_lab2.4 → **chem_lab6.1.html**（让位记忆回廊），Ⅲ 其他卡片 href 已同步
- Ⅳ 复习板块现为 2 卡：知识星图 + 记忆回廊
- zao v18（113 文件 14081KB）验收：hash=index-BpBlvQUB.js 一致；浏览器探针：THREE r128、画布在、crumb 四链接正确、备案在；lab6.1 200
